...
The RPM spec file provided here is for Gpunion-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://gpunion.io/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Gpunion git repository](https://github.com/GpunionProject/Gpuniontoken) but are not part of
...
Currently only `Source10` is used, for the example `gpunion.conf` file.
...
in the gpunion git repository. At present only an SVG version of the Gpunion
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the gpunion
...
needed when 0.12.1 is released, a proper fix is already in the Gpunion git
master branch.
...
idea to build Gpunion Core against a static version of that library compiled
...
The `gpunion.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `gpunion-server` package. At this point, most RPM based
...
The `gpunion-server` package should have SELinux support. How to properly do
...
Gpunion-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/gpunion/). He has been
packaging Gpunion for Fedora at least since 2012.
...
4. I renamed the `gpunion` package that contains the Qt GUI to `gpunion-core` as
that appears to be how the general population refers to it, in contrast to
`gpunion-xt` or `gpunion-classic`. I wanted to make sure the general population

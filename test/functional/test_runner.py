...
`test/functional/test_framework/test_framework.py:GpunionTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/gpunion_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_gpuniond = config["components"].getboolean("ENABLE_GPUNIOND")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_utils and enable_gpuniond):
        print("No functional tests to run. Wallet, utils, and gpuniond must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    # Warn if gpuniond is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "gpuniond"]) is not None:
            print("%sWARNING!%s There is already a gpuniond process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "GPUNIOND" not in os.environ:
        os.environ["GPUNIOND"] = build_dir + '/src/gpuniond' + exeext
        os.environ["GPUNIONCLI"] = build_dir + '/src/gpunion-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        # In case there is a graveyard of zombie gpunionds, we can apply a
...
class RPCCoverage():
    ...
    commands per `gpunion-cli help` (`rpc_interface.txt`).
